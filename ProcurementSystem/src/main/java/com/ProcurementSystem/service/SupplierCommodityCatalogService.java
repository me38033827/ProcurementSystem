...
public class SupplierCommodityCatalogService {
	...
	public void insertCommodityCatalog(CommodityCatalog commodityCatalog) {
		supplierSystemCommodityCatalogDao.insertCommodityCatalog(commodityCatalog);
	}
		...
		try {
			...
			for (int i = 0; i < rows; i++) {// 寻找数据起始标志-“DATA”
				...
				if (cell.getContents().equals("DATA")) {
					...
					supplierSystemCommodityCatalogDao.setIsActivated(commodityCatalog);// 维护商品目录状态属性
					commodityCatalog.setItemCount(commodityCatalog.getCommodities().size());
					supplierSystemCommodityCatalogDao.setItemCount(commodityCatalog);// 维护商品目录商品数量属性
				}
			}
