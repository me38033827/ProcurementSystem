...
(function($, window, document, undefined) {
	...
	Tree.prototype.clickHandler = function(event) {
		...
		if ((classList.indexOf('expand-icon') !== -1)) {
			...
			if (classList.indexOf('sim-create-question') !== -1) {
				...
				var id = target.attr('class').split(" ")[2].split("-")[1];
				editQuestion(id);
			} else if (classList[1].indexOf('folder-create') !== -1) {
				var id = classList[1].split("-")[2];
				createFolder(id);
			} else if (classList[1].indexOf('template-create') !== -1) {
				var id = classList[1].split("-")[2];
				createTemplate(id);
			} else if (classList[1].indexOf('template-open') !== -1) {
				var id = classList[1].split("-")[2];
				openTemplate(id);
			} else if (classList[1].indexOf('templateTaskPhase-create') !== -1) {
				var id = classList[1].split("-")[2];
				createTemplateTaskPhase(id);
			} else if (classList[1].indexOf('templateTaskSchedule-create-') !== -1) {
				var id = classList[1].split("-")[2];
				createTemplateTaskSchedule(id);
			} else if (classList[1].indexOf('templateTaskSchedule-edit-') !== -1) {
				var id = classList[1].split("-")[2];
				templateTaskScheduleEdit(id);
			} else if (classList[1].indexOf('templateTaskSchedule-delete-') !== -1) {
				var id = classList[1].split("-")[2];
				templateTaskScheduleDelete(id);
			} else if (classList[1].indexOf('template-delete-') !== -1) {
				var id = classList[1].split("-")[2];
				deleteTemplate(id);
			} else if (classList[1].indexOf('templateTaskPhase-edit') !== -1) {
				var id = classList[1].split("-")[2];
				templateTaskPhaseEdit(id);
			} else if (classList[1].indexOf('templateTaskPhase-delete') !== -1) {
				var id = classList[1].split("-")[2];
				templateTaskPhaseDelete(id);
			} else if (classList[1].indexOf('folder-edit') !== -1) {
				var id = classList[1].split("-")[2];
				folderEdit(id);
			} else if (classList[1].indexOf('folder-delete') !== -1) {
				var id = classList[1].split("-")[2];
				folderDelete(id);
			} else if (classList[1].indexOf('folder-profile') !== -1) {
				var id = classList[1].split("-")[2];
				folderProfile(id);
			} else if (classList[1].indexOf('templateTaskSchedule-info') !== -1) {
				var id = classList[1].split("-")[2];
				templateTaskScheduleInfo(id);
	...
	Tree.prototype.injectStyle = function() {
		...
		if (this.options.injectStyle && !document.getElementById(this.styleId)) {
			$(
					'<style type="text/css" id="' + this.styleId + '"> '
							+ this.buildStyle() + ' </style>').appendTo('head');
		}
	};
	...
	Tree.prototype.identifyNode = function(identifier) {
		return ((typeof identifier) === 'number') ? this.nodes[identifier]
				: identifier;
	};
