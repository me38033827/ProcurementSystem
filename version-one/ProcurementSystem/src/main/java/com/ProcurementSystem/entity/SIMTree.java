...
public class SIMTree {
	...
	public String treeviewQuestionnaireAssembler(SIMTreeNode parentNode) {
		String resultHTMLString = "";	// hold the final html string
			...
			for (SIMTreeNode child : children){
				...
				if(child.getType()==2){
					...
					if(child_type==2 && child_accept_value==2){						
						answer+="<li style=\"width: 220px\"> <div class=\"select\"> <select data-role=\"select\" id=\"ans-"+child.getSupplierSIM().getId()+"\" name=\"ans-"+child.getSupplierSIM().getId()+"\">";
						answer+="<option selected=\"selected\" disabled=\"disabled\" style='display:none' value=''></option>";
						
						List<SupplierSIMSelection> selections = child.getSupplierSIM().getSelections();						
						
						for(SupplierSIMSelection selection: selections){
							answer = answer +  "<option value=\""+selection.getContent()+"\">"+selection.getContent()+"</option>";
						}
						answer=answer+"</select> </li>";
					}
					if(child_type==2 && child_accept_value!=2 && child_answer_type==1){
						answer="<li style=\"width: 220px\"> <div class=\"input\"> <input type=\"text\" data-role=\"input\" data-role-input=\"true\" id=\"ans-"+child.getSupplierSIM().getId()+"\" name=\"ans-"+child.getSupplierSIM().getId()+"\"/> </div> </li>";
					}
					
				}
				if (title_flag == false) {
					resultHTMLString += "<li data-caption=\""+child.getSupplierSIM().getTitle()+"\" class=\"expanded\"> <span class=\"caption\">"+child.getSupplierSIM().getTitle()+"</span> <ul>"+answer;
					title_flag = true;
				}else {
					// 新node
					resultHTMLString += "<li data-caption=\""+child.getSupplierSIM().getTitle()+"\" class=\"expanded\"> <span class=\"caption\">"+child.getSupplierSIM().getTitle()+"</span> <ul>"+answer;
				...
				if(child.getType()==1){
					resultHTMLString += treeviewQuestionnaireAssembler(child);
				}
				resultHTMLString += "</ul> <span class=\"node-toggle\"></span> </li>";
			}
			...
			for (SIMTreeNode child : children){
				...
				if(child.getType()==2){
					...
					if(child_type==2 && child_accept_value==2){
						...
						stateObj.put("checked", true);
						stateObj.put("expanded", true);
						jsonObj.put("state", stateObj);
					}
					if(child_type==2 && child_accept_value!=2 && child_answer_type==1){
	...
	// 加载树形调查问卷, string
	public JSONObject traverseToStringWithoutSelection() {
		Queue<SIMTreeNode> queue = new LinkedList<>();
		queue.offer(root);
		SIMTreeNode a = queue.poll();
		//return traverseHelperWithoutSelection(a);
		String res = treeviewQuestionnaireAssembler(a);
